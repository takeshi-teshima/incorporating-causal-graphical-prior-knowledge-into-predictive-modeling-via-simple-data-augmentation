...
class CausalDataAugmentationEagerTrainingExperimentAPI:
    ...
    def _augment(self, data: pd.DataFrame, graph: GraphType, augmenter_config: AugmenterConfig) -> Tuple[pd.DataFrame, np.ndarray]:
    ...
    def run_method_and_eval(self, data: pd.DataFrame, graph: GraphType,
        ...
        try:
            if isinstance(self.aug_coeff, float):
                ...
                augmented_data, aug_weights = self._augment(data, graph, self.augmenter_config)
